...
import {
  ...
  forgetCachedHtmlImage,
  pruneCachedHtmlImages,
} from "~/utils/zineImageCache";
...
export function useZineStore() {
  const state = useState<ZineState>(
    "mini-zine-a4-state",
    createInitialZineState,
  );
  const cachePruneWatchRegistered = useState(
    "mini-zine-a4-cache-prune-watch-registered",
    () => false,
  );
  ...
  async function addImageElement(
    file: File,
    pageId: PageId = state.value.selectedPageId,
  ): Promise<ImageInsertResult> {
    if (!import.meta.client) {
      return {
        ok: false,
        error: "La carga de imágenes solo está disponible en el navegador.",
      };
    }
    ...
    if (!file.type.startsWith("image/")) {
      return {
        ok: false,
        error: "El archivo seleccionado no parece ser una imagen compatible.",
      };
    }
  ...
  async function addImageElements(
    files: File[],
  ): Promise<ImageBatchInsertResult> {
    ...
    for (const file of files) {
      ...
      if (!insertResult.ok) {
        ...
        result.errors.push(
          insertResult.error ?? "No se pudo cargar un archivo.",
        );
        continue;
      }
    ...
    if (result.overflowCount > 0) {
      result.warnings.push(
        "Algunas imágenes se omitieron porque no quedaban páginas disponibles.",
      );
    }
  ...
  function deleteElement(id: string) {
    ...
    state.value.pageElementIds[current.pageId] = state.value.pageElementIds[
      current.pageId
    ].filter((item) => item !== id);
